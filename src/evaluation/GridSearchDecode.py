...
def run_single_evaluation(config, data_output_list, game_managers, gan_output_list, grid_search_output,
    ...
    for levelIdx, level in enumerate(level_list):
        print(levelIdx, ": ", level.get_level_metadata())
...
def run_evaluation_xml_levels(level_folder_path, parameter, start_game=True):
    config = Config.get_instance()
    ...
    print(game_manager.copy_game_levels(
        level_path=level_folder_path,
        rescue_level=False
    ))

    game_manager.simulate_all_levels(start_idx = 3, end_idx = 3 + level_count, wait_for_stable = True,
                                     wait_for_response = False)
